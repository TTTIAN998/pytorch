...
class TestPackage(torch._inductor.test_case.TestCase):
    ...
    def test_install_survives_stale_cleanup_hooks(self):
        # The first compile installs its generated functions into the module
        # globals behind a CleanupHook keyed on the generated code object.
        # install() rebinds the same names, so a hook firing afterwards must not
        # delete the freshly installed binding.
        ctx = DiskDynamoStore()
        ...
        prefixes = ("__compiled_fn", "__resume_at")
        scope = fn.__globals__
        preexisting = {name for name in scope if name.startswith(prefixes)}
        others = [ref() for ref in list(CleanupManager.instance.refs.values())]
        other_ids = {id(code) for code in others}
        ...
        pinned = [
            ...
            if idx not in other_ids
        ]
        pinned_ids = {id(code) for code in pinned}
