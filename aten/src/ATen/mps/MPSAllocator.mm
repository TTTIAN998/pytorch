  ...
  void parseArgs(const std::string& env) {
    ...
    for (size_t i = 0; i < tokenizer.size(); i++) {
      ...
      if (key == "mps_large_alloc_threshold_mb") {
        ...
        TORCH_CHECK(shared_keys.find(key) != shared_keys.end(), "Unrecognized key '", key, "' in MPS allocator config.");
...
BufferBlock* MPSHeapAllocatorImpl::alloc_buffer_block(size_t size, uint32_t usage) {
  ...
  if (solo_threshold > 0 && size >= solo_threshold) {
    ...
    BufferBlock* buffer_block = new BufferBlock(alloc_size, size, buf, /*heap=*/nullptr);
